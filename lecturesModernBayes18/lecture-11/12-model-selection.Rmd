...
$$p(\lambda^{-1}, \bbeta \mid y, X) = p(\lambda^{-1} \mid y, X) \times p(\bbeta \mid y,X, \lambda^{-1})$$

Goal: simulate $(\lambda^{-1}, \beta) \sim p(\lambda^{-1}, \bbeta \mid y, X)$
Starting value $(\beta_0, \lambda_0)$
1. Simulate $$\lambda^{-1} \sim p(\lambda^{-1} \mid y, X)$$ Gives us  $(\lambda_1^{-1}, \beta_0)$
2. Use this updated value of $\lambda_1^{-1}$ to simulate 
$$\beta \sim p(\bbeta \mid y,X, \lambda^{-1})$$ Gives us $(\lambda_1^{-1}, \beta_1)$
...
\beta \mid \lambda MVN(0, g (X^TX)^{-1})\\
\lambda^{-1} &\sim \Ga(\nu_0/2,\nu_0 \lambda^{-1}/2)
\end{align}
...
===
Suppose that we are content with a estiamte of $\bbeta$ from which we can make predictions.
...
Obtain $P(\beta_j \neq \mid y)$ as well as posterior confidence intervals for all of the parameters. Compare our results to that in part (a.)
...
for(s in 1:S){
  ...
  if (pm>0){
    Hg = (g/(g+1)) * Xs[,z==1,drop=F] %*% solve(t(Xs[,z==1,drop=F]) %*% Xs[,z==1,drop=F]) %*% 
      t(Xs[,z==1,drop=F])
    
    s20=summary(lm(ys ~ -1+Xs[,z==1,drop=F]))$sigma^2 # estimated residual variance from OLS
  }
