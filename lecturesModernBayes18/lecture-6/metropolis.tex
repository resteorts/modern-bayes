...
\title{Intro to Markov Chain Monte Carlo: Part II}
...
%First introduced in 1953 by (give names).
...
``If we take samples $X_o, X_1, X_2, \ldots$ then by the ergodic theorem, they will eventually reach $\pi,$
which is known as the stationary distribution (the true pmf)."
...
\frame{
\frametitle{Metropolis Algorithm Setup} 

%First introduced in 1953 by (give names).

Let the proposal matrix be a stochastic matrix (non-negative and all rows sum to 1). 
That is, $$Q = (Q_{ab}: a,b \in \mathcal{X}).$$
\vskip 1em

Note: Often we can't often compute $\pi(x)$ efficiently. However, we can often compute $$\pi(x) = \frac{\tilde{\pi}(x)}{z}, z>0.$$

\vskip 1em

What is computationally difficult to compute is often $z$ in practice. 
...
\begin{enumerate}
\item Choose a symmetric proposal matrix $Q.$ $(Q_{ab} = Q_{ba} \forall a,b)$
\item Initialize $x_o \in \mathcal{X}.$
\item for $i=0,1,\ldots n-1:$
\begin{itemize}
\item Sample \textcolor{blue}{proposal $x$} from $Q(x_i, x)$ (i.e. $p(x \mid x_i) = Q(x_i, x).$).
\item Sample $u$ from Uniform(0,1)
\item If $$u < \dfrac{\tilde{\pi}(x)}{ \tilde{\pi}(x_i)}$$
then we accept $x_{i+1} = x.$
\newline
Else, $x_{i+1} = x_i.$
\end{itemize}
\end{enumerate}
...
b u^{1/a} &= x 
\end{align*}
and thus, we can sample from $\Mono(a,b)$ by drawing $U\sim \Uniform(0,1)$ and setting $X=b U^{1/a}$.\footnote{ It turns out that this is an inverse of the Pareto distribution, in the sense that if $X\sim\Pareto(\alpha,c)$ then $1/X\sim\Mono(\alpha,1/c)$. }
...
\frame{
 ...
 \textbf{Scatterplot}. 
 \vskip 1em
 Scatterplot visualizes the posterior distribution $p(\alpha, c|x_{1:n})$.
  \vskip 1em
