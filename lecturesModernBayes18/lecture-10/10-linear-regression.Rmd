...
- Back to oxygen uptake example\footnote{Much of this lecture comes from the Hoff book.}
...
- Can we predict $\bY$ as a function of each row in the matrix $X_{n\times p}$ denoted by $\bx_j.$
- Which $\bx_j$'s have an effect?

Such question can be assessed via a linear regression model $p(\by \mid X).$
...
\}\\
&= (2\pi \sigma^2 )^{-n/2} \exp\{ (\by - X\bbeta)^T(\by - X\bbeta)\}
\end{align}
...
$$p(\bbeta \mid \by, X) \propto MVN(\bbeta \mid \by, X, \Lambda^{-1})
$$
where
$\Lambda = a X^TX +bI$
and $\mu = a \Lambda^{-1} X^T \by.$

Posterior computation (details)
===
\begin{align}
& p(\bbeta \mid \by, X) \\
& \propto \exp\{ -\frac{a}{2} (\by-X\bbeta)^T (\by-X\bbeta)\}
\times \exp\{ -\frac{b}{2}\bbeta^T \bbeta)\}\\
&\propto \exp\{ -\frac{a}{2}[\textcolor{red}{\by^T\by} -2\bbeta^TX^T\by + \bbeta^TX^TX\bbeta] -\frac{b}{2}\bbeta^T \bbeta
\}\\
&\propto \exp\{ a\bbeta^TX^T\by  -\frac{a}{2}\bbeta^TX^TX\bbeta -b/2\bbeta^T \bbeta
\}\\
&\propto \exp\{ a\bbeta^T[X^T\by] -1/2\bbeta^T(aX^TX + bI)\bbeta
\}
\end{align}
