...
Hard Discs in a Box Example
===
Have an $X = (x,y)$ coordinate for each molecule.
- Remark: For the rest of the lecture, $X$ will denote the two coordinate vectors of the molecule. 
\vskip 1em
...
theta<-0 ; delta<-2 ; S<-500 ; THETA<-NULL
set.seed(1)
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.5]{metropolis_normal-500}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[scale=0.5]{metropolis_normal-10000}
...
Given that we have looked at n=500 iteration of the Metroplis sampler, does it seem that out approximation is a good one? 
