...
===
$$\bm{Y}_i \mid \btheta, \Sigma \sim MVN(\btheta_i, \Sigma).$$ 
$$ \btheta_i \sim MVN(\bm{\mu}_0, \Lambda_0)$$
$$ \Sigma \sim \text{inverseWishart}(\nu_o, S_o^{-1}).$$
...
Using the samples from the Gibbs sampler, we have generated 5,000 samples $$(\bm{\theta}^{1}, \Sigma^(1), \ldots,\bm{\theta}^{5000}, \Sigma^(5000) )$$ that approxmiates $p(\theta, \Sigma \mid y_1, \ldots, y_n).$
