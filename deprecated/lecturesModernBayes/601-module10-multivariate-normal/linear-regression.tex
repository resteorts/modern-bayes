...
&p(y\mid x,D)\\
&=
\int p(y\mid x,D,w) p(w\mid x,D) dw\\
&= \int p(y\mid x,w) p(w\mid D) dw \\
&=\int  N(y\mid w^Tx, a^{-1}) N(w \mid \mu, \Lambda^{-1}) dw\\
&\propto
\int \exp\{ -a/2(y-w^Tx)^2\} 
\exp\{-1/2(w-\mu)^T \Lambda (w- \mu)\} dw \\
&\propto
\int \exp\{
-a/2(y^2 -2(w^Tx)y +(w^Tx)^2)\notag \\
&-1/2(w^T\Lambda w - 2w^T\Lambda \mu + \mu^T\Lambda \mu)\} 
\end{align}
...
\begin{enumerate}
\item First show that the above, can be written as 
$\int N(w\mid -) g(y) dw$ as give the parameters of the Gaussian. 
\item Next, show trivially that 
$\int N(w\mid -) g(y) dw = g(y)$ with the parameters in 1.
\item Finally, show that $ g(y) \propto N(y \mid -)$
and give the parameters for the normal of the predictive distribution. 
\end{enumerate}
To summarize, you should in the end find that 
$$p(y \mid x, D) \propto \exp\{-\lambda/2(y-u)^2)$$ where
$\lambda = a(1-ax^TL^{-1}x), u = \lambda^{-1}ax^TL^{-1}\Lambda \mu$, $L = axx^T + \Lambda$. (You may assume that $w$ has mean $m$).
...
\begin{itemize}
\item When does $\hat{\theta}^B = \hat{\theta}^{MLE}$?
(What does this mean in terms of the prior variance or precision)? 
\item Suppose $\hat{\theta}^B \neq \hat{\theta}^{MLE}$.
What benefit are we getting from linear regression in this case
over ordinary least squares? Explain. 
\end{itemize}
