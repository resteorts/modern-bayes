...
# Line plots of histogram of measured heights - male and female
...
plot(midpoints(edges),female,"b",label="female",linewidth=2)
plot(midpoints(edges),male,"r",label="male",linewidth=2)
plot(midpoints(edges),combined,"k--",label="combined",linewidth=2)
...
xlim(lower,upper)
grid(true)
legend(numpoints=1,loc=2,labelspacing=0.1,fontsize=15) #names,fontsize=fontsize,numpoints=1,ncol=ncol)
...
# Females
M_f = M(x,lambda_f,mu_0_f,lambda_0_f)
L_f = L(x,lambda_f,mu_0_f,lambda_0_f)
# Males
M_m = M(y,lambda_m,mu_0_m,lambda_0_m)
L_m = L(y,lambda_m,mu_0_m,lambda_0_m)
