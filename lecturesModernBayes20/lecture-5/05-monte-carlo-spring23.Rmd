...
\textbf{Example}: Suppose the $h(\cdot)$ is the identity function.

\vspace*{1em}

$$E_X[h(x)] = E_X[X] = \textcolor{red}{\int_X x f(x)\; dx}$$


Sampling Methods
===

Solution: Draw samples $X_1, \ldots, X_n \stackrel{iid}{\sim} f(x).$ 

$$E_X[h(x)] \approx  \frac{1}{n}\sum_{j=1}^n h(X_j).$$
...
Simulate a N(0,1) histogram, and the mean and variance. Compare it to the density in R. Do this for 10 versus 1000 simulations. 
...
What do you observe empirically?
...
\vskip 1em
The classical way to solve this is generate samples $(X_1,\ldots,X_n)$ from $f.$ 
\vskip 1 em
Now propose as an approximation the empirical average:
$$E_f[h(x)] \approx \frac{1}{n}\sum_{j=1}^n h(x_j) := \bar{h}_n.$$ 
\vskip 1 em
...
===
Also, under certain assumptions\footnote{see Casella and Robert, page 65, for details}, the asymptotic variance $v_n$ can be approximated and then can be estimated from the sample $X = (X_1,\ldots,X_n)$ by 
$$v_n = V[\frac{1}{n} \sum_j h(x_j) ]= \frac{1}{n^2}\sum_j V[h(x_j)] \approx
\textcolor{red}{\frac{1}{n^2}} \sum_{j=1}^n [h(x_j) - \bar{h}_n]^2.$$ Finally, by the CLT (for large $n$), 
...
===
\textcolor{red}{Assume that $X_i$ are iid observations. Suppose h is the identity function.}
