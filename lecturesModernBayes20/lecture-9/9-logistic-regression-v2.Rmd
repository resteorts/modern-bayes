...
Before, we can introduce the general terminology of the GLM, we first must know about 
...
where $\theta$ and $\phi$ are the **natural and dispersion parameters**, respectively; $a,b,c$ are functions.

Connection to GLMs
===

In a GLM, we are allowed to assume that $Y \mid X$ has a probability density function or probability mass function of the form of equation~\ref{eqn:exponential}.
...
- We assume $E[Y\mid X] = \mu$ and our goal is to estimate $\mu.$

- We usually assume that $\phi$ is known. 

- The systematic component relates $\eta$ to $X.$ 

In a GLM, $$\eta = \beta^T X = \beta_1 X_1 + \ldots \beta_p X_p$$

The link component connects the random and systematic components, via a link function $g.$ The link function provides a connection between $\mu$ and $\eta.$ 
...
- $\theta$ is a parameter we use it to govern the shape of the density $Y\mid X.$ Thus, $\mu$ depends on $\theta.$

- $\phi$ is a dispersion parameter of the density, which we assume is known

- For GLM's, $\eta = \beta^T X= \beta_1 X_1 + \ldots \beta_p X_p.$ 

Our goal is aiming to model a transformation of the mean $\mu$ by a function of $X$:
$$g(\mu) = \eta(X).$$
...
Show that $Y\mid X$ is an exponential family. 
...
\} \\
&= \exp\{
y \log (\textcolor{blue}{\frac{p}{1-p}}) + \log(1-p)
\} \\
\end{align}
