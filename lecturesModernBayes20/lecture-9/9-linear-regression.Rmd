...
Exam III is **December 2, during class** (open note/open book)
...
\begin{align}
y_i &\mid \bbeta \sim \text{MVN}(X \bbeta, \sigma^2 I)\\
...
\sum_{i=1}^n
({y}_i - X\bbeta)^T(\bm{y}_i - X\bbeta)
\} \\
&\propto \exp\{\frac{-1}{2\sigma^2}
\sum_{i=1}^n
[
{y}_i^T{y}_i + \bbeta^TX^TX\bbeta - 2\bbeta^TX^T{y}_i
]
\}\\
&\propto \exp\{\frac{-1}{2\sigma^2}
\sum_{i=1}^n
[
\bbeta^TX^TX\bbeta - 2\bbeta^TX^T{y}_i
]
\}\\
&\propto \exp\{-\frac{1}{2}
[
\bbeta^T\textcolor{blue}{\frac{n\;X^TX}{\sigma^2}}\bbeta - 2\bbeta^T\textcolor{red}{\frac{nX^T\bar{{y}}}{\sigma^2}}
]
\}
\end{align} 

This implies that $A_1 = \frac{n\;X^TX}{\sigma^2}$ and $\textcolor{red}{b_1 = \frac{nX^T\bar{{y}}}{\sigma^2}}.$
...
$$b_n = b_o + b_1 = \Sigma_0^{-1}\bbeta_0 + \frac{nX^T\bar{{y}}}{\sigma^2}.$$
$$\bbeta \mid \bm{y} \sim MVN(A_n^{-1} b_n, A_n^{-1}) =: MVN(\bbeta_n, \Sigma_n),$$

where $$\mu_n = (\Sigma_0^{-1} + \frac{n\;X^TX}{\sigma^2})^{-1}(\Sigma_0^{-1}\bbeta_0 + \frac{nX^T\bar{{y}}}{\sigma^2})$$ and
...
$$\bbeta_n = E[\bbeta\ \mid \bm{y}, \bX, \sigma^2] = (\Sigma_o^{-1} + (nX^TX)/\sigma^2)^{-1}
(\Sigma_o^{-1}\bbeta_0 + nX^T\bar{y}/\sigma^2)$$
