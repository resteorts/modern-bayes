...
where the link function is $$g(\mu_i) = \mu_i.$$
and the variance function is $$V(\mu_i) = 1.$$
...
- Then $E(Y_i/n_i) = p_i$ and $Var(Y_i/n_i) = \dfrac{pi(1-p_i)}{n}.$
...
- $\theta$ is a parameter we use to govern the shape of the density $Y\mid X.$ Thus, $\mu$ depends on $\theta.$

- $\phi$ is a dispersion parameter of the density, which we will assume is known

- For GLM's, $\eta = \beta^T X.$ Our goal is aiming to model a transformation of the mean $\mu$ by a function of $X$:
$$g(\mu) = \eta(X).$$
