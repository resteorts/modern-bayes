...
Applications: IQ scores, snow fall or rain fall 
...
\textcolor{blue}{One can show that the joint posterior of $(\mu, \lambda \mid x_{1:n})$ is}
\begin{align}\label{equation:NormalGamma-posterior}
\mu,\lambda|x_{1:n}\,\sim\,\NormalGamma(M,C,A,B)
\end{align}
...
\lambda^{\textcolor{blue}{a}-1/2}\exp\Big(-\tfrac{1}{2}\lambda(\textcolor{blue}{c}\mu^2 - 2 \textcolor{blue}{c m}\mu + \textcolor{blue}{c m^2} + \textcolor{blue}{2 b})\Big).\label{equation:NormalGamma}
\end{align}
...
& = \lambda^{\textcolor{blue}{a+n/2}-1/2}
\exp\Big(-\tfrac{1}{2}\lambda\big((\textcolor{blue}{c+n})\mu^2 - 2 (\textcolor{blue}{c m +\textstyle\sum x_i}) \mu + \textcolor{blue}{c m^2 + 2 b + \sum x_i^2}\big)\Big)\\
&\overset{\text{(a)}}{=}\lambda^{\textcolor{blue}{A}-1/2}\exp\Big(-\tfrac{1}{2}\lambda\big(\textcolor{blue}{C}\mu^2 - 2 \textcolor{blue}{C M} \mu + \textcolor{blue}{\textcolor{blue}{C M^2} + 2 B}\big)\Big)\\
...
$$\,C M = (c m + \textstyle\sum x_i)$$
$$C M^2 + 2 B = c m^2 + 2 b + \sum x_i^2.$$


Solving for $M$ and $B$, (verify this on your own), we can find that $$M = (c m + \sum x_i)/(c+n)$$ and $$B = b +\tfrac{1}{2}(c m^2-C M^2+\textstyle\sum x_i^2).$$
...
- At the start of the year, students are given an IQ test.
- Researchers randomly selected 20 percent of students in each class. 
- Students have been grouped in sputers (outstanding) versus control. 
- The test was given at the end of the year. 
- The change in IQ score is measured for spurters versus control groups. 
...
The average increase in IQ score is larger for the spurters (see previous plot). 

How strongly do these data support the hypothesis taht the teachers' expectations caused the spurters to perform better than their classmates? 

The Pygmalion effect
===
Plot the **number of students** versus the **change in IQ score** for the two groups. 

How strongly do these data support the hypothesis that the teachers' expectations caused the spurters to perform better than their classmates (i.e., the Pygmalion effect)? 
...
\end{align*}
where $(\mu_C, \mu_S, \lambda_S^{-1}, \lambda_C^{-1})$ are unknown parameters. 

- We are interested in the difference between the means---in particular, is $\mu_S>\mu_C$?

The Bayesian approach computes the posterior probability that $\mu_S>\mu_C$:
$$ \Pr(\bm\mu_S > \bm\mu_C \mid x_{1:n_S},y_{1:n_C}). $$
- To finalize our model, we need to specify priors on the parameters. 
...
\end{align*}
where we assume $(m,c,a,b)$ are fixed and known. 

Hyper-parameter settings
===
\begin{itemize}
\item $m = 0$ (Don't know whether students will improve or not, on average.)
\item $c = 1$ (Prior certainty is equivalent to $c$ data points.)
\item $a = 1/2$ (Prior certainty is equivalent to $2a$ data points.)
...
Other commons priors for the mean/precision
===

- The Normal-Gamma priors induces a strong dependency between $\mu$ and $\lambda$ (which may not be warranted). See the plot of the samples from the prior regarding the Pygmalion effect. 

- It is more common to assume the distributions of $\mu$ and $\lambda$ are independent apriori:

$$\lambda \sim \text{Gamma}(a,b) \quad \text{and}, \quad \mu \sim N(\mu, s^2).$$


\begin{itemize}
\item These are not conjugate priors, but conditionally conjugate in the sense that:
\begin{itemize}
\item for any fixed $\lambda$, it is conjugate for $\mu$
\item for any fixed, $\mu$, it is conjugate for $\lambda$.
\end{itemize}
\end{itemize}

- Why are these useful? They are easy to work with in situations with Markov chain Monte Carlo (soon to come) to approximate the posterior.
