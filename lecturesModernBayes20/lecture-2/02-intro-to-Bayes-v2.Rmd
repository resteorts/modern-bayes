...
$(\Theta, \mathcal{A}, \ell)$ define a game, where typically the goal is to minimize one's loss (or win the game). 
...
Example: Nature decides it will rain all day long. 
...
The confusion matrix illustrates a loss function $\ell(\theta, a)$ for all combinations of $a,\theta.$
...
$$ R(\theta,\delta(X)) = \E\big(\ell(\theta,\delta(X))\mid \theta) = \int \ell(\theta,\delta(x))\,p(x|\theta)\,dx.$$
...
$$\rho(\theta, \delta(X)) = \E\big(\ell(\theta,\delta(X))\mid X = x) = \int \ell(\theta,\delta(X)) \; p(\theta \mid x) \; d\theta,$$

if $\Theta$ is continuous, while the integral is replaced with a sum if $\Theta$ is discrete.


Motivating Example (Return)
===

1. Given our motivating example, calculate the frequentist risk and Bayes risk.

2. Suppose it always rains. Find the frequentist risk and Bayes risk.

3. What is the action that minimizes the frequentist risk and Bayes risk.
...
A \term{Bayes rule} is an optimal decision procedure $\hat{\delta}(X)$ minimizes the posterior risk for all values of $x \in \mathcal{X}.\footnote{Sometimes the loss is restricted to be nonnegative, to avoid certain pathologies.}$
...
&= 
\E(\theta^2 - 2\theta \;{\delta}(x))  + {\delta}(x)) |x_{1:n})\\
...
Because the loss function is convex, $\delta(x) = \E(\theta|x_{1:n})$ is the unique solution. 
