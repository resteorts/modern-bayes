...
- We choose the prior here very randomly or naively. What happened? Can you think of a simple fix?
...
```{r}
posterior_pred <- function(a, b, x, n, new_point) {
  if (new_point == 1) {
    return((a + x)/(a + b + n))
  }else {
    return(b + (1 - x)/(a + b + n))
...
===
```{r, echo=FALSE}
plot(0, posterior_pred(a = 2, b = 6, x = 6, 
                n = 10, new_point = 0), lty = 2, xlim = c(-0.5,1.5), ylim = c(-0.5,12), xlab="binary outcome (0,1)", ylab="posterior predictive distribution", type = "h", lwd = 2)
points(1, posterior_pred(a = 2, b = 6, x = 6, 
                n = 10, new_point = 1), type = "h", lwd = 2)
