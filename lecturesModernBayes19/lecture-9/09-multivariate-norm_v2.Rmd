...
We may be interested in the population mean $\bm{\mu}_{\p times 1} = X_{n \times p} \bm{\beta}_{p \times 1}:$
...
- A row of $\bm{X}_{n \times p}$ represents a covariate we might be interested in, such as the age of a person.
...
and $\Sigma_{p \times p} = \var(\bm{y}) = E[(\bm{y}_{p\times1} - \bmu_{p \times 1})_{p\times1}(\bm{y}_{p\times1} - \bmu_{p \times 1})^T_{1\times p}],$ 
where 
\[  \bmu_{p \times 1}= \left( \begin{array}{c}
\mu_1\\
\mu_2\\
