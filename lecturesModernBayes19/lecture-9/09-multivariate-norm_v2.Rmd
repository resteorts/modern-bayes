...
We may be interested in the population mean ${\mu}_{\p times 1} = X_{n \times p} \bm{\beta}_{p \times 1}:$
...
and $\Sigma_{p \times p} = \var(\bm{y}) = E[(\bm{y}_{p\times1} - \mu_{p \times 1})_{p\times1}(\bm{y}_{p\times1} - \mu_{p \times 1})^T_{1\times p}],$ 
where 
\[  \mu_{p \times 1}= \left( \begin{array}{c}
\mu_1\\
\mu_2\\
\vdots\\
\mu_p
