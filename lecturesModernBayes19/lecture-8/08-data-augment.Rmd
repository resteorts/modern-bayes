...
\begin{align}
  & X_1,\ldots, X_n \mid \mu,\pi\ \sim F(\mu,\pi)\\
      & \mu:=(\mu_0,\mu_1) \stackrel{iid}{\sim} \N(m,\ell^{-1})\\
        & \pi \sim \Beta(a,b),
\end{align}
where $m,\ell$ and $a,b$ are assumed known.
...
\begin{figure}
\begin{center}
\includegraphics[width=0.9\textwidth]{examples/mix-p_trace-a.png}
\includegraphics[width=0.9\textwidth]{examples/mix-p_trace-b}
\caption{Traceplots of the mixture weight, $\pi$. What do you notice about the two different runs?}
\label{figure:mixb}
\end{center}  
\end{figure}
