...
Agenda
===
- Gibbs sampling (Three-stage sampler)
- Gibbs sampling (multi-stage sampler)
- Gibbs sampling with an application to missing data (censoring)
...
Posterior inference 
===
\textcolor{blue}{Goal: find $p(\theta,z_3, z_8, z_9, z_{10}, z_{12} |x_{1:n})$?}
...
2. To sample from $p(\theta,z_3, z_8, z_9, z_{10}, z_{12} |x_{1:n})$, we cycle through each of the full conditional distributions,
\textcolor{blue}{
\begin{align*}
\theta &\mid z_{1:n}, x_{1:n}\\
z_3 &\mid \theta, z_{-3}, x_{1:n}\\
z_8 &\mid \theta, z_{-8}, x_{1:n}\\
z_9 &\mid \theta, z_{-9}, x_{1:n}\\
z_{10} &\mid \theta, z_{-10}, x_{1:n}\\
z_{12} &\mid \theta, z_{-12}, x_{1:n}
\end{align*}
}
...
\end{align}
where the last line holds by conditional independence. 
...
\textcolor{blue}{
$$
p(z_i|z_{-i},x_{1:n},\theta) \propto p(z_i|\theta)p(x_i|z_i).
$$}
...
Heart Disease (Censoring) Application
===

Let's recall the motivating application behind our model, which a study on 12 patients that have heart disease. 

Recall that the number of years before death for each is
$$3.4, 2.9, 1.2+, 1.4, 3.2, 1.8, 4.6, 1.7+, 2.0+, 1.4+, 2.8, 0.6+$$
where the $+$ indicates that the patient was alive after $x$ years, but the researchers lost contact with the patient after that point in time. 
...
```{r}
# input the data
# z's are the observed values
z <- c(3.4,2.9,1.4,3.2,1.8,4.6,2.8)
# c's are the censored values
...
```{r}
# Gibbs sampler
# z is the fully observed data
# c is the censored data
...
```{r}
set.seed(5983)
# Z follows a Gamma(shape = r, scale= \theta)
# the scale is slightly more peaked
r <- 10
# theta follows a Gamma(1,1)
# Setting a,b both to one is a default prior on theta
a <- 1
b <- 1
n.iter <- 10000
init.theta <- 1
...
Using the data and functions given to you (posted to Sakai with Homework 6), investigate the following questions: 

1. Write code to produce traceplots and running average plots for the censored values for 40 iterations. Do these diagnostic plots suggest that you have run the sampler long enough? Explain. 
